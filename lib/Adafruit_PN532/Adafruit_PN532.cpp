...
// ---- FeliCa functions (patched for ChipperZero) --------------------------------
...
bool Adafruit_PN532::felica_Poll(uint8_t sysH, uint8_t sysL,
  ...
  // [4]      = ~length+1
  // [5]      = D5 (TFI)
  // [6]      = 4B (InListPassiveTarget response)
  // [7]      = NbTg (number of targets)
  // [8]      = Tg
  // [9]      = NbFe (= SENSF_RES length, includes this byte; e.g. 0x13 w/ sysCode)
  // [10]     = 01h (SENSF_RES response code)
  // [11..18] = IDm
  // [19..26] = PMm
...
bool Adafruit_PN532::felica_ReadWithoutEncryption(const uint8_t *idm,
                                                  ...
                                                  uint8_t blockNum,
                                                  uint8_t *blockData) {
  // Build ReadWithoutEncryption FeliCa frame (16 bytes)
  uint8_t cmd[16];
  cmd[0]  = 16;                          // frame length
  cmd[1]  = 0x06;                        // ReadWithoutEncryption command code
  memcpy(&cmd[2], idm, 8);              // IDm
  cmd[10] = 0x01;                        // NumServices = 1
  cmd[11] = serviceCode & 0xFF;          // service code LSB
  cmd[12] = (serviceCode >> 8) & 0xFF;  // service code MSB
  cmd[13] = 0x01;                        // NumBlocks = 1
  cmd[14] = 0x80;                        // block descriptor: 2-byte form
  cmd[15] = blockNum;                    // block number

  uint8_t resp[32];
  uint8_t respLen = sizeof(resp);
  if (!inDataExchange(cmd, sizeof(cmd), resp, &respLen)) return false;

  // Response: [0]=len, [1]=07h, [2..9]=IDm, [10]=Flag1, [11]=Flag2,
  //           [12]=NumBlocks, [13..28]=block data (16 bytes)
  if (respLen < 29)        return false;
  if (resp[10] != 0x00)   return false;  // StatusFlag1 error

  if (blockData) memcpy(blockData, resp + 13, 16);
  return true;
}
